...
# sortie de run_segmentation.py : organoïde bright-field (fond noir). La région
# organoïde est dérivée des pixels non-noirs (np.asarray(m, bool) plus bas).
MASKS_ROOT = str(Path(DATASET_ROOT).parent / f"{Path(DATASET_ROOT).name}_brightfield_SAM3")
...
def load_gray_frames(tif_path):
    ...
    try:
        ...
        it = [arr] if arr.ndim == 2 else list(arr)
    except Exception:
        from PIL import Image, ImageSequence
        it = [np.asarray(p) for p in ImageSequence.Iterator(Image.open(tif_path))]
    frames = []
    for a in it:
        a = np.asarray(a)
        if a.ndim > 2:
